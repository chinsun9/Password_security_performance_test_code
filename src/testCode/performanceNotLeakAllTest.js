...
async function performanceTest() {
    ...
    for (let i = 0; i < notLeakString.length; i++) {
        ...
        testPassword = notLeakString[i];
        testLeakCount = 1;

        fs.appendFileSync(
            __dirname + '/../../files/performanceNotLeakAllTestLog.log',
        ...
        await passwordClient.passwordValidation(testPassword).then(function (result) {
            ...
            fs.appendFileSync(__dirname + '/../../files/performanceNotLeakAllTestLog.log', `유출 여부 예측: ${result.predictPoint < 0.6 ? '유출된 비밀번호' : '유출되지 않은 비밀번호'}\n`, 'utf8');
            ...
            if ((testLeakCount == 0 ? true : false) == (result.predictPoint < 0.6 ? true : false)) {
                fs.appendFileSync(__dirname + '/../../files/performanceNotLeakAllTestLog.log', `=== 예측 성공 여부: ${'예측 성공'} ===\n\n`, 'utf8');
                console.log(`=== 예측 성공 여부: ${'예측 성공'} ===\n\n`);

                success++;
            } else {
                fs.appendFileSync(__dirname + '/../../files/performanceNotLeakAllTestLog.log', `=== 예측 성공 여부: ${'예측 실패'} ===\n\n`, 'utf8');
    ...
    fs.appendFileSync(__dirname + '/../../files/performanceNotLeakAllTestLog.log', `예측 성공: ${success}회, 예측 실패: ${fail}회, 예측 정확도 ${(success / (success + fail)) * 100}%`, 'utf8');
